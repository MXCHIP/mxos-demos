...
merr_t bt_packet_pool_init( bt_packet_pool_t* pool, uint32_t packet_count, uint32_t header_size, uint32_t data_size )
{
    ...
    return mxos_rtos_init_mutex( &pool->mutex );
}
...
merr_t bt_packet_pool_deinit( bt_packet_pool_t* pool )
{
    ...
    mxos_rtos_deinit_mutex( &pool->mutex );
    ...
    for ( a = 0; a < packet_count; a++ )
    {
        ...
        if ( linked_list_remove_node_from_rear( &pool->pool_list, &removed_node ) == mxos_BT_SUCCESS )
        {
...
merr_t bt_packet_pool_allocate_packet( bt_packet_pool_t* pool, bt_packet_t** packet )
{
    ...
    mxos_rtos_lock_mutex( &pool->mutex );
    ...
    {
        result = mxos_BT_PACKET_POOL_EXHAUSTED;
    }

    mxos_rtos_unlock_mutex( &pool->mutex );
...
merr_t bt_packet_pool_free_packet( bt_packet_t* packet )
{
    ...
    else if ( packet->packet_id == PACKET_ID )
    {
        ...
        mxos_rtos_lock_mutex( &packet->pool->mutex );
        ...
        mxos_rtos_unlock_mutex( &packet->pool->mutex );
